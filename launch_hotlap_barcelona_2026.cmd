@echo off
REM ===============================================================
REM  ONE-CLICK: BARCELONA HOTLAP 2026 — MERCEDES-AMG W16 (ANTONELLI)
REM  - Backs up cfg/race.ini, installs the preset, fires Crew Chief,
REM    runs acs.exe, and kills CC when AC exits.
REM  - Pre-writes the light-fuel quali setup (Barcelona_Quali_2026,
REM    14 L = 1 out lap + 3 flying laps) to last.ini (both the
REM    ks_barcelona and generic folders) so hotlap mode (TYPE=4)
...
set "PRESET=%ACDOC%\cfg\hotlap_barcelona_2026.ini"
set "TARGET=%ACDOC%\cfg\race.ini"
set "BACKUP=%ACDOC%\cfg\race.ini.bak"
set "SETUPSRC=%ACDOC%\setups\rss_formula_hybrid_2025_alpine\ks_barcelona\Barcelona_Quali_2026.ini"
set "SETUPTRK=%ACDOC%\setups\rss_formula_hybrid_2025_alpine\ks_barcelona\last.ini"
...
echo  =============================================================
echo  [BARCELONA HOTLAP 2026 - MERCEDES-AMG W16]
echo.
...
REM --- Force-load the light-fuel quali setup (hotlap mode reads last.ini)
if exist "%SETUPSRC%" (
    echo Loading Barcelona quali setup ^(14 L, 1 out + 3 laps^)...
    copy /Y "%SETUPSRC%" "%SETUPTRK%" >nul
    copy /Y "%SETUPSRC%" "%SETUPGEN%" >nul
) else (
    echo WARNING: Barcelona quali setup not found, AC will use its last-used setup:
    echo   %SETUPSRC%
)
